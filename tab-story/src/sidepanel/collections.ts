...
export async function addToCollection(id: number, sources: { url: string; title: string; favicon?: string }[]) {
  await db.transaction('rw', db.collections, db.tabs, db.folders, async () => {
    ...
    for (const source of sources) {
      ...
      if (tab?.deletedAt) await db.tabs.update(tab.id!, { deletedAt: undefined });
      if (!tab) {
        ...
        const tabId = Number(await db.tabs.add({ url: source.url, title: source.title || source.url, favicon: getFavicon(source.favicon, domain), domain, folderId: folder.id, tags: [], notes: '', pinned: false, createdAt: Date.now() }));
        members.add(tabId);
        continue;
      }
      members.add(tab!.id!);
